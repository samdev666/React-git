...
import { LOCKUP_PLAN_ACTION, PLAN_ACTION } from "../../redux/actions";
import {
  LOCKUP_PLAN,
...
const validators = {
  financialYear: [
    required(
      messages?.measure?.financialOverview?.lockup?.planForm?.validators
        ?.financialYear
    ),
...
const PlanForm: React.FC<Props> = ({ onCancel, onSuccess }) => {
  ...
  const { entity: planEntity, applyFilters } = usePagination<LockupPlanEntity>(
    {
      ...paginatedPlans,
      api: LOCKUP_PLAN_FILTER_LISTING_API.replace(":tenantId", tenantId),
    },
    ...
    return new Promise<void>((resolve, reject) => {
      reduxDispatch(
        apiCall(
          addPlanConfig?.type === UserActionType.EDIT
            ? `${LOCKUP_PLAN}/${addPlanConfig?.planInformation?.id}`
            : TENANT_LOCKUP,
          resolve,
          reject,
          addPlanConfig?.type === UserActionType.EDIT
      ...
      .then(() => {
        ...
        toast(
          <Toast
            text={
              messages?.measure?.financialOverview?.lockup?.planForm?.success?.[
                addPlanConfig?.type === UserActionType.EDIT
                  ? "updated"
                  : "created"
              ]
        ...
        <>
          ...
          <FormRow>
            <FormRowItem>
              {connectField("title", {
                label:
                  messages?.measure?.financialOverview?.lockup?.planForm?.title,
      ...
      {submitError && (
        <FormRow>
          <FormRowItem>
            <FormError
              message={
                messages?.measure?.financialOverview?.lockup?.planForm?.error
